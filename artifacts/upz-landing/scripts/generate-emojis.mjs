...
let body = `// Auto-generated by scripts/generate-emojis.mjs. Do not edit manually.
...
export const GENERATED_EMOJI_CATEGORIES = ${JSON.stringify(Array.from(new Set(emojis.map((e) => e.category).filter(Boolean).sort())))} as const satisfies readonly string[];
`;

// Per-category JSON files for code-split loading
const byCategory = new Map();
for (const emoji of emojis) {
  const cat = emoji.category || "uncategorized";
  if (!byCategory.has(cat)) byCategory.set(cat, []);
  byCategory.get(cat).push(emoji);
}
for (const [category, items] of byCategory) {
  const safeName = category.replace(/[^a-z0-9-]/gi, "_").toLowerCase();
  const catFile = path.join(projectRoot, "src", "data", `emojis.${safeName}.json`);
  await writeFile(catFile, `${JSON.stringify(items, null, 2)}\n`, "utf8");
}
...
for (const cat of sortedCategories) {
  ...
  body += `  "${cat}": () => import("./emojis.${safeName}.json") as Promise<{ default: GeneratedEmoji[] }>,\n`;
}
body += `};\n`;
...
console.log(`Generated ${emojis.length} emojis (${Math.round(totalBytes / 1024)} KB) -> ${path.relative(projectRoot, outputJsonFile)}  (${sortedCategories.length} categories)`);
